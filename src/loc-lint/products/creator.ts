...
export function createCreatorProduct(root?: string): LocLintProduct {
  const repo = productRoot(REPO, root);
  const creatorCore = path.join(repo, "packages", "survey-creator-core");
  const registries = buildRegistries(creatorCore);
  const renderer = (name: string) => path.join(repo, "packages", name, "src");
  return {
    name: PRODUCT,
